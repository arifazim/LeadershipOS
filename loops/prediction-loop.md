...
5. **Log outcome** — `memory/lessons/` — this is the loop most directly tied to `kaizen/weekly-review.md` Q1; a wrong prediction logged here should be checked before the next forecast is made, not just recorded after the fact
...
- **One-shot forecasting**: treating every forecast as independent instead of checking `memory/lessons/` for a prior miscalibration on the same type of estimate
